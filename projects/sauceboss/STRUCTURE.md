...
| GET | `/api/v1/sauceboss/sauces/{sauce_id}/export.json` | None | Download a single sauce as a versioned JSON envelope (`{version, exportedAt, sauce}`). |
...
| POST | `/api/v1/sauceboss/sauces/import` | JWT | Multipart `file` upload (single-sauce JSON). Creates a sauce owned by the current user; missing `parentSauceId` returns a non-fatal warning. |
