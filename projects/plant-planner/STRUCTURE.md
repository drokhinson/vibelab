...
│   ├── library.js       — "My Plants" library view (status filter + detail panel + assign-to-planter picker)
│   ├── browser.js       — "Plant Browser" view (catalog search + filter dropdown + plant/favorite buttons)
│   ├── import.js        — "Import plants" view (standalone API-fill orchestration)
...
shared-backend/routes/plant_planner/  — FastAPI route package
  ├── api_clients.py     — Perenual fetcher (species-list + species-details) + record normalization
...
- **plantplanner_plant_cache** — Phase-1 API-backed catalog. id (uuid PK), source (`perenual`; legacy rows may also carry `trefle` / `merged`), source_id (text), scientific_name (text UNIQUE), common_name (text), family (text), emoji (text), hardiness_min/max (int — USDA zone), sunlight (`full_sun`/`part_shade`/`full_shade`), watering (`frequent`/`average`/`minimum`/`none`), cycle (`annual`/`perennial`/`biennial`), indoor (bool), height_min_cm/height_max_cm (int — from Perenual `dimensions`), spread_cm (int — from Perenual `dimensions`), days_to_harvest (int; legacy Trefle column), edible (bool), vegetable (bool), toxicity (text — derived from Perenual `poisonous_to_humans`/`poisonous_to_pets`), growth_rate (text), ph_min/ph_max (real; legacy Trefle), sowing (text; legacy Trefle), nitrogen_fixation (bool; legacy Trefle), tags (text[]). **Three image sizes** mirrored to Supabase Storage: image_thumbnail_url/path, image_medium_url/path, image_regular_url/path — all nullable. raw_perenual_json keeps the full upstream payload (description, care_level, soil[], growing_months[], attracts[], dimensions, etc.) for the popup to read. raw_trefle_json is preserved on legacy rows but no longer written. last_synced_at + last_image_synced_at timestamps.
...
- `GET  /api/v1/plant_planner/catalog/search` — Cache-first plant search. Accepts every wizard input as a query param: `shade_level`, `water_plan`, `usda_zone`, `planting_season`, `garden_type`, `grid_width`, `grid_height` plus optional overrides `query`, `edible`, `indoor`, `cycle`, `max_height_cm`, `max_spread_cm`, `planter_size`. The route maps `garden_type + grid_*` to a small/medium/large bucket and applies height/spread caps so a 12-inch pot never returns 10-foot tomatoes. Returns cached `plantplanner_plant_cache` rows; if hits < threshold, lazy-fills from Perenual (species-list filtered to the conditions, then per-row species-details hydration capped at 5 concurrent calls) and persists.
- `GET  /api/v1/plant_planner/catalog/{cache_id}` — Single cached plant; lazy-enriches missing hardiness via Perenual on first call.
- `POST /api/v1/plant_planner/catalog/fill/perenual` — Step 2 of the new-planter loading orchestration. Body is the wizard conditions (`FillBody`). Calls Perenual species-list with the wizard filters, then fans out species-details lookups (semaphore=5) so each row carries hardiness, dimensions → height/spread, growth_rate, edible/vegetable/toxicity, and the rich `raw_perenual_json` payload (description, care_level, soil[], growing_months[], attracts[], etc.). Returns `{ status, fetched, new_plants }`.
- `POST /api/v1/plant_planner/catalog/fill/compatible` — Step 3. Applies the wizard's full filter against the now-enriched cache and returns `{ status, compatible_plants, total_in_cache }`.
...
"My Plants" (library, default after sign-in), and "My Gardens" (planter
...
My Gardens View (list saved gardens, "+ New Garden")
    ↓ ("+ New Garden")
New-Garden Wizard (state.js → currentView === "wizard") — 4 steps
  Step 1 — Filters             (light · water · hardiness zone optional · planting season; chip rows reused
                                from the browser's filter component)
  Step 2 — Planter             (type · size · name; live preview)
  Step 3 — Catalog source      ("Sync from API" runs the 3-step fill orchestration before browsing;
                                "Use existing catalog" skips it and goes straight to /catalog/search)
  Step 4 — Review & confirm    (read-only summary; "Continue to plant selection" CTA)
    ↓ (on Confirm — POST /gardens, then openShoppingForGarden(id, {runFillSequence: <choice>}))
Plant Shopping View (currentView === "shopping")
  ↳ For new planters: 3-step API to-do list overlay first, then grid:
       1. Save planter (already done; rendered as ✓ on arrival)
       2. POST /catalog/fill/perenual    — fetched X / Y new (species-list + species-details fan-out)
       3. POST /catalog/fill/compatible  — N plants compatible with planter
       ↳ "Continue to plant selection" button on completion → grid loads
...
"Plant Browser" navbar button)
  ├── Search row: free-text input · "Filters" disclosure · "Import" button
  ├── Filters dropdown panel (collapsible; sunlight, watering, hardiness
  │   zone, cycle, edible chip rows; live-applied to /catalog/search; "Clear
  │   all" resets state). Active count is shown on the disclosure button.
  ├── Pinterest-style grid (reuses .shopping-card markup + styles)
  │     ↳ Two action buttons per card:
  │         • Leaf — "Add to plant list"     (POST/PUT user_plants status=current)
  │         • Heart — "Add to favorite list" (POST/PUT user_plants status=wishlist)
  │       The two are mutually exclusive (UNIQUE on user_id+plant_cache_id);
  │       tapping a button when already in that status DELETEs the row.
  └── Card tap → slide-in detail panel; CTAs match the two card buttons.

Plant Import View (currentView === "import", reachable from the browser's
"Import" button)
  ├── Form: light · water · season chip rows + optional zone + edible toggle
  └── On "Run import" — runs the 2-step orchestration:
      1) POST /catalog/fill/perenual    (species-list + species-details fan-out)
      2) POST /catalog/fill/compatible
      ↳ "Back to browser" button on completion bounces to the browser with
        the import filters preselected so the new species are immediately
        visible. No planter is saved.

My Planters Library — assign-to-planter
  Detail panel adds an "Add to a planter" picker beneath the existing
  "In planters" chips. The picker lists every garden the plant is NOT
  already in. Tapping a chip POSTs to
  /gardens/{garden_id}/shortlist/{plant_cache_id} (atomic on the server).
  After success, the user_plants list is re-fetched so the chip moves into
  "In planters" on the next render.
```
...
| `PERENUAL_API_KEY` | shared-backend (Railway) | Sole upstream source for the cache. Both wizard fill and lazy fill call species-list with the wizard filters, then fan out species-details lookups (semaphore=5) to populate the rich fields the popup shows. |
...
- 2026-05-10 — **Perenual-only fill (Trefle + Flora removed).** The cache pipeline is now single-source. `POST /catalog/fill/perenual` and the lazy-fill path in `/catalog/search` both call Perenual species-list with the wizard filters, then fan out species-details lookups capped at 5 concurrent calls (`_PERENUAL_DETAILS_CONCURRENCY` in `catalog_routes.py`). The details payload populates the rich fields the popup needs (family, hardiness, dimensions, description, care_level, soil[], growing_months[], attracts[], …) — previously these were null because only species-list was called. `normalize_perenual` was extended to map `dimensions` → `height_min/max_cm` + `spread_cm`, `growth_rate` direct, `edible_fruit`/`edible_leaf` → `edible`, `type == 'vegetable'` → `vegetable`, and `poisonous_to_humans`/`poisonous_to_pets` → `toxicity` ("toxic to humans, pets"). The image-mirror call was dropped from `fill_perenual` (the popup falls back to Perenual's CDN URL when the Storage path is missing). Removed: `POST /catalog/{cache_id}/enrich/trefle`, `POST /catalog/fill/trefle`, `POST /catalog/fill/flora`; all Trefle/Flora client + normalizer code in `api_clients.py`; `TREFLE_API_TOKEN` and `FLORA_API_KEY` env vars; the wizard's Trefle/Flora fill steps; the popup's "Import extra info from Trefle" CTA + helpers (`_anyTrefleExtraMissing`, `_plantTrefleImportButtonHtml`, `trefleEnrich`); the "Native range" / "Introduced range" chip rows. The Trefle-derived DB columns (`raw_trefle_json`, `nitrogen_fixation`, etc.) and the corresponding popup bullets remain — they just stop being written and render as "—" until backfilled from Perenual on a re-fill.
- 2026-05-10 — **My Plants (rename back) + per-plant Trefle import.** The middle navbar tab is renamed back to "My Plants" (was "My Planters" briefly during the three-tab cutover); the underlying view and endpoints are unchanged. Detail panels in both Plant Browser and My Plants now show plant info in two stacked groups: a Core block (sunlight, watering, cycle, hardiness, edible) and an Extra info block (height, days_to_harvest, soil pH, toxicity, growth_rate, sowing). The Extra block is sourced from Trefle's nested `growth` / `specifications` data, which is often missing on Perenual-only / Flora-only cache rows. When the extras are sparse the panel surfaces an "Import extra info from Trefle" button that calls a new `POST /catalog/{cache_id}/enrich/trefle` endpoint. Backend changes: (a) new `_trefle_lookup_for_row` helper that searches Trefle by scientific_name and falls back to common_name (the wizard's `fill_trefle` step also picks up the fallback); (b) a new `_trefle_record_with_detail` helper that always calls `trefle_get(id)` after the search hit so growth/specifications nested fields are normalized — without this the Trefle pass returned near-empty extras. Frontend adds shared `_coreInfoBullets` / `_trefleExtraBullets` / `_trefleExtrasMissing` / `_renderDetailBullets` / `trefleEnrich` helpers in `web/helpers.js` so both panels render consistently from one source of truth.

- 2026-05-10 — **Browser filters + Import view + 4-step wizard + assign-to-planter.** Four interlocking changes layered on top of the new Plant Browser: (1) Browser gains a collapsible Filters dropdown (sunlight, watering, hardiness, cycle, edible) above the grid plus an Import button. Each card now has TWO action buttons — leaf "Add to plant list" (status=current) and heart "Add to favorite list" (status=wishlist), mutually exclusive via the existing UNIQUE(user_id, plant_cache_id). (2) New `web/import.js` (`currentView === "import"`) runs the same Trefle/Perenual/Flora/compatible orchestration the wizard runs, but standalone — `runFillStep` / `setFillStep` / `renderFillProgress` were factored out of `web/shopping.js` into `web/helpers.js` so both views share one runner. After import completes, the user bounces back to the browser with the import filters preselected. (3) The new-garden wizard collapses to four steps: Filters (light + water + zone + season — chips on one screen) → Planter (type + size + name combined) → Sync-or-cache choice (the user opts in/out of the API fill before the grid loads) → Review. The conditional location-skip for indoor planters is replaced by an always-shown but optional zone field; `submitGardenWizard` now passes the user's sync choice through `openShoppingForGarden`. Edit links on review retarget the new step numbers. (4) The library detail panel adds an "Add to a planter" picker that lists planters not already containing the plant; tapping a chip calls a new atomic endpoint `POST /gardens/{garden_id}/shortlist/{plant_cache_id}` (in `garden_routes.py`) that appends the cache_id and idempotently upserts the matching wishlist row via the existing `upsert_wishlist_rows` helper. No DB migration; everything else is client-only.

- 2026-05-10 — **Three-tab navigation: Plant Browser added, My Plants renamed to My Planters.** Navbar now exposes three top-level views instead of two. New `web/browser.js` (`currentView === 'browser'`) renders a standalone catalog browse — free-text search against `/catalog/search` with no wizard filters — reusing the `.shopping-card` / `.shopping-detail-*` markup. Heart toggle posts to `/user_plants` (`status: "wishlist"`) on add and `DELETE /user_plants/{id}` on remove; library membership is fetched once on view entry and tracked client-side as `plant_cache_id → user_plant_id` for the toggle. The legacy "My Plants" library tab is renamed to "My Plants" (label only — endpoints, table, and behaviour are unchanged). No backend or schema changes.
