...
> Last updated: 2026-05-10 (Browser filters + Import view + 4-step wizard + assign-to-planter)
...
- `POST /api/v1/plant_planner/catalog/{cache_id}/enrich/trefle` — Per-plant Trefle lookup-and-merge. Searches Trefle by scientific_name (falls back to common_name), hydrates via `trefle_get` so the nested `growth` / `specifications` fields are captured, merges into the cache row, and returns the updated `CatalogPlant`. Powers the "Import extra info from Trefle" button in the plant detail panel for plants that arrived via Perenual/Flora and are missing height/pH/days_to_harvest/toxicity/growth_rate/sowing.
- `POST /api/v1/plant_planner/catalog/fill/trefle` — Step 2 of the new-planter loading orchestration. Body is the wizard conditions (`FillBody`). Runs a Trefle search/filter for those conditions and upserts results into the cache. Returns `{ status, fetched, new_plants }`.
...
New-Garden Wizard (state.js → currentView === "wizard") — 4 steps
  ...
  Step 3 — Catalog source      ("Sync from API" runs the 5-step fill orchestration before browsing;
...
Plant Shopping View (currentView === "shopping")
  ↳ For new planters: 5-step API to-do list overlay first, then grid:
       1. Save planter (already done; rendered as ✓ on arrival)
       2. POST /catalog/fill/trefle      — fetched X / Y new
       3. POST /catalog/fill/perenual    — enriched X of Y
       4. POST /catalog/fill/flora       — enriched X of Y (FLORA_API_KEY required)
       5. POST /catalog/fill/compatible  — N plants compatible with planter
...
"Import" button)
  ...
  └── On "Run import" — runs the 4-step orchestration:
      1) POST /catalog/fill/perenual
      2) POST /catalog/fill/trefle
      3) POST /catalog/fill/flora       (FLORA_API_KEY required; fail-soft)
      4) POST /catalog/fill/compatible
...
| `FLORA_API_KEY` | shared-backend (Railway) | **Required** by step 4 of the new-planter loading orchestration (`POST /catalog/fill/flora`) |
