// geoZones — the IANA-timezone → country table.
//
// A HAND-MAINTAINED COPY of web/domain/geo-data.js. The two must stay in step;
// there is no build pipeline between them (same arrangement as the duplicated
// assets in .claude/rules/assets.md). Regenerate both from the same command,
// which is documented in the web file's header.
//
// FORMAT. One line per country: the ISO 3166-1 alpha-2 code, then every IANA
// zone in it, space-separated. models/geo.js inverts it once at first use.
...
// Zones tzdata retired but platforms still report. See the web copy's header.
export const ALIAS_ZONES = `
Africa/Asmera ER
