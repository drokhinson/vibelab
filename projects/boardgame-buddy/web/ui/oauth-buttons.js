// ui/oauth-buttons.js — canonical OAuth button markup per .claude/rules/auth-ui.md.
...
(function () {
  ...
  function oauthButtons({ disabled = false, onGoogle = "", onApple = "" } = {}) {
    const dis = disabled ? "disabled" : "";
    return `
      <button type="button" class="auth-oauth-btn auth-oauth-google" ${onGoogle ? `onclick="${onGoogle}"` : ""} ${dis}>
        ${GOOGLE_SVG}<span>Continue with Google</span>
      </button>
      <button type="button" class="auth-oauth-btn auth-oauth-apple" ${onApple ? `onclick="${onApple}"` : ""} ${dis}>
        ${APPLE_SVG}<span>Continue with Apple</span>
      </button>
