...
(function () {
  ...
  function renderPlayCard(card) {
    ...
    // Migration 013: the card carries the whole play, so hand it to the domain
  ...
  function renderBack(card, s) {
    ...
    // Since migration 013 the feed card carries the full roster with scores
    ...
    if (!p) {
      // No seed: either an RPC that predates 013, or a surface whose card was
  ...
  const controller = {
    ...
    async toggle(playId) {
      ...
      // the scores and the rounds (migration 013), so the flip is pure paint.
