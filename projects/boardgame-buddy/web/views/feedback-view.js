...
//
// Reached from two Settings rows (Add feedback / Report a bug) and open to every
// signed-in user, not just admins. Structurally this is the sibling of
// admin-reports-view.js — a spoke off Settings with a filter above a list — with
// three differences worth naming up front:
//
...
//      for exactly two things: whether the Open/Resolved toggle renders, and
//      whether each row gets a Resolve button. A non-admin sees a working
//      screen, which is the whole point. The server is the actual gate — it
//      forces status=open for a non-admin whatever this screen sends.
//
//   2. THE BOARD IS ORDERED BY LIKES, AND LIKING DOES NOT RE-ORDER IT. The sort
//      happens in bgb_feedback_list, on load. Re-sorting on a tap would move the
//      row out from under the finger that tapped it.
//
//   3. IT CAN OPEN WITH THE COMPOSE SHEET UP. `?compose=1` opens it empty,
//      `?compose=bug` opens it with the type preselected — that is what makes
//      the Settings "Report a bug" row a shortcut rather than a second screen.
//      The param is stripped with replaceUrl immediately, so a refresh or a
//      back-then-forward lands on the board rather than re-opening the sheet.
...
(function () {
  class FeedbackView extends window.View {
     ...
     * logout → login and back-stack pops, so a previous session's filters and
     * items would otherwise render under the next account's screen
     ...
     *
     * Put on top rather than sorted into place: it is what the person just
     * wrote, and burying it under the popular items would read as the submit
     * having failed. The next load puts it where its like count belongs. It is
     * dropped entirely when the current filters exclude it — pretending
     * otherwise would show a row that vanishes on the next refresh.
     */
    _onSubmitted(item) {
      if (!item) return;
      const shown = this._status === "open"
        && (!this._type || this._type === item.feedback_type)
        && (!this._topic || this._topic === item.topic);
      if (shown) this._items = [item, ...this._items];
      this._loaded = true;
      this.render();
    }
    ...
    _renderHead() {
      return `
        <header class="spoke-head">
          ...
          <button class="spoke-head__add btn btn-primary btn-sm" type="button"
    ...
    _renderFilters() {
      ...
      // A .spoke-toggle, not the .theme-seg from Settings: that control reads
      // PAPER tokens because it lives inside a .set-card, and this screen is
      // chrome (.claude/rules/theming.md §6).
      const statusToggle = admin ? `
        <div class="feedback__status">
          <span class="feedback__status-label" id="feedback-status-label">Showing:</span>
          <div class="spoke-toggle" role="tablist" aria-labelledby="feedback-status-label">
            ${["open", "resolved"].map((s) => `
              <button class="spoke-toggle__pill ${this._status === s ? "is-active" : ""}"
                      role="tab" aria-selected="${this._status === s}"
                      onclick="window.feedbackView._setStatus('${s}')">
                <span class="spoke-toggle__label">${s === "open" ? "Open" : "Resolved"}</span>
              </button>`).join("")}
          </div>
        </div>` : "";
      ...
      return `
        ${statusToggle}
        ${this._renderChips("type", this._types, this._type, "All types")}
        ${this._renderChips("topic", this._topics, this._topic, "All topics")}
      `;
    }

    /**
     * One horizontally-scrollable chip rail per filter axis, built from the
     * lookup rows rather than a hardcoded list — which is the point of the
     * options living in the database. Renders nothing until they land.
     */
    _renderChips(axis, options, active, allLabel) {
      ...
      const chip = (id, label, icon) => `
        <button class="chapter-add__chip ${active === id ? "chapter-add__chip--active" : ""}"
                ...
                onclick="window.feedbackView._setFilter('${axis}', '${escapeAttr(id)}')">
          ${icon ? `<i data-icon="${escapeAttr(icon)}" class="w-3.5 h-3.5"></i>` : ""}
          ${escapeHtml(label)}
        </button>`;
      return `
        <div class="chapter-add__filter-chips" role="tablist"
             aria-label="Filter by ${escapeAttr(axis)}">
          ${chip("", allLabel, "")}
          ${options.map((o) => chip(o.id, o.label, o.icon)).join("")}
        </div>`;
    }
    ...
    _renderEmpty() {
      const filtered = !!(this._type || this._topic);
      ...
      return `
        <li class="feedback__item" data-feedback-id="${escapeAttr(item.id)}"
          ...
          <div class="feedback__marks">
            ...
            <span class="feedback__tag">
              <i data-icon="${escapeAttr(item.topic_icon || "circle")}" class="w-3.5 h-3.5"></i>
              ${escapeHtml(item.topic_label || item.topic)}
            </span>
          </div>
     ...
     * resolve can have removed a row above it between the paint and the patch.
     */
    _patchRow(item) {
    ...
    _setFilter(axis, value) {
      const key = axis === "type" ? "_type" : "_topic";
