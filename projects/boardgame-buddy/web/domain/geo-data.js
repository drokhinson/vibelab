// @ts-check
// domain/geo-data.js — the IANA-timezone → country table, and nothing else.
//
// Split out of domain/geo.js so the logic stays readable: this file is data,
// it is only ever regenerated, and it should never be edited by hand for any
// reason other than a tzdata update.
//
// SOURCE. Generated from the tzdata distribution's `zone.tab` (the canonical
// zone list, one row per country + zone). Regenerate with:
//
//   awk -F'\t' '!/^#/ && NF>=3 {print $1, $3}' /usr/share/zoneinfo/zone.tab \
//     | sort | awk '{ a[$1] = a[$1] " " $2 } END { for (c in a) print c a[c] }' \
//     | sort
//
// FORMAT. One line per country: the ISO 3166-1 alpha-2 code, then every IANA
// zone in it, space-separated. Grouped by country rather than written as a
// flat {zone: country} object because it is a third of the bytes and it is
// the direction a human reads it in; domain/geo.js inverts it once at load.
//
// This table is also the app's country LIST — the picker in Settle Up is built
// from its keys. That is deliberate: one source, so a country can never be
// offered that no timezone could ever detect, and a detected country is always
// one the picker can display. It costs the handful of ISO entries with no
// resident population (Bouvet Island and friends), which is not a loss for a
// "where did you play this game" field.
(function () {
  window.BGB_GEO_DATA = {
    // "CC Zone Zone …" per line.
    zonesByCountry: `
AD Europe/Andorra
AE Asia/Dubai
AF Asia/Kabul
...
`,
    ...
    // Zones tzdata retired but browsers still report — a "backward" link is
    // kept forever precisely because systems out there hold the old name, and
    // `Intl.DateTimeFormat().resolvedOptions().timeZone` hands back whatever
    // the platform has. Chrome shipped "Asia/Calcutta" for years. These are
    // the ones with real user populations behind them; anything missed falls
    // through to the locale fallback rather than being detected wrong.
    aliasZones: `
Africa/Asmera ER
