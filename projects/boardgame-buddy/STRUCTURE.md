...
- `POST /api/v1/boardgame_buddy/bgg/link` — body `{username}`; store the handle on the profile. Does NOT call BGG (their XMLAPI rate-limits unauthenticated reads of other users' data, so we defer the round-trip to sync). Invalid handles surface as a 404 from `/bgg/sync` when BGG replies with `<errors>`. Returns `{bgg_username}`.
