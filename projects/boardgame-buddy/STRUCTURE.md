...
**Expansions section.** Rendered on every base game — on the Game Detail page (a reel of expansion polaroids) and on the host's Gather card (a collapsible toggle list). Both carry an **Import expansions** button, and it stays available when nothing is imported yet. It opens `widgets/import-expansions-modal.js`, which lists the expansions BoardGameGeek links to this game that BgB doesn't have (`GET /games/{id}/expansions/available`); each row's name has the base game's name stripped off the front, and a **+** imports it into the catalog and pins it to this base game. Import is catalog-only — it doesn't touch the caller's collection. Since expansions are hidden from search, this popup is the only way one enters the catalog, so the base game has to be imported first.
