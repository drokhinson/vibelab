...
- Session create/join/get (and every session endpoint's response payload) are single Postgres RPCs as of migration 036 (`bgb_create_session` / `bgb_join_session` / `bgb_get_session` / `bgb_session_bundle`) — previously 4-6 sequential PostgREST round trips per request, which made host/join taps and the 2s lobby poll crawl at cross-region RTTs. See `db/functions/boardgamebuddy.sql`.
