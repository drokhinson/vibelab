...
## What It Does
A Strava-style log for board game plays. The home view is a chronological feed of plays from the user and their accepted buddies, interspersed with "hot games this week", suggested buddies, and dormant games from the user's own collection. Logging a play is a guided three-screen cascade — Gather → Play → Settle Up — that walks the host through the play and mirrors read-only to non-host joiners (who can score their own column live). The Play tab is split in two: hosting on top (host a game, replay the last one, play offline, or browse the Game Explorer) and joining on the bottom (enter a short code, or pick a live session hosted by a buddy). Hosting also works with no connectivity at all — the cascade runs from cache, the play queues on the device, and it uploads on the next online session. Profiles are fully public and show a Strava-style stats strip + collection grid. The reference-guide system is fully user-driven: each user builds their own per-game guide by adding "chapters" — either creating new ones or browsing the community pool. The pool sorts by popularity. Reports on offensive chapters route to admin review.
...
## Tech Stack
- **Frontend (web):** Vanilla HTML/CSS/JS, DaisyUI v4 + Tailwind CDN, Lucide icons, Supabase JS SDK (CDN). Installable PWA (`manifest.json` + `sw.js`) so the host flow works with no connectivity — see Offline mode under Screen Flow §3.
...
1. Auth (login/signup) → splash → 2. **Feed** (home): chronological mix of plays from the viewer and their accepted buddies, plus inline "hot this week" / "buddies you may know" / "time to revisit" rails. A search pill at the top opens the **Game Search** screen. **Pull down from the top to refresh:** the gesture re-fetches the first page and splices only the plays that weren't already on screen in above the ones that were, so the pages the user scrolled in, their position, and the rails all survive — the indicator reports what came back ("2 new plays" / "You're up to date"). It falls back to a full swap in the one case where prepending would lie: nothing on the fresh page overlapped what was held, meaning more landed than a page can carry.
3. **Log a play**: the Play tab is split — **Host on top, Join on the bottom**.
   ...
   **Offline mode.** The same three screens with the entire lobby subtracted — no `POST /sessions`, no code, no 2s poll, no Realtime, no phase PATCH, no photo. All of that exists to serve joiners, and there can't be any without a server. What remains already ran locally: the game picker and player picker paint from `bgbCache` (`game.recent` / `buddy:all`, warmed by `/bootstrap` at 24h fresh / 7d stale), the draft lives in localStorage, and Save hands the play to `domain/outbox.js` instead of the API.
   - **Entering.** `domain/net.js` (`window.BgbNet`) is the single source of truth, combining three signals: a user-chosen "Play offline" card on the Play tab (sticky, persisted to `bgb_offline_v1`), `navigator.onLine === false`, and two consecutive `fetch` network failures. Only the last two are automatic — `navigator.onLine` reports true on a captive portal and on one bar that times out rather than fails, which is exactly the venue this feature is for, hence the manual card. `PlayFlowView` latches the answer once at mount so its guards can't disagree mid-cascade; **Save is the one place that re-checks**, so a host who walked back into signal gets a live write.
