...
The badge catalog — fifteen rows, seeded by migration 061. Kept as **data, not a
...
| metric | TEXT | which computed metric drives it — CHECKed against the nine `bgb_sync_achievements` computes, so a badge can't name a metric nothing calculates |
...
| `/profile/achievements` | `achievements` | — | — | Achievements spoke: a progress ring over fifteen medallion badges in four groups (At the table / Victories / The reference guide / Making it yours). Locked badges keep their art, desaturated, and print what they still want from you; tapping any badge opens its detail sheet. One call. |
