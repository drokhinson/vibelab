...
- **The game picker answers from the device first; the network only refines it.** Every keystroke in `widgets/game-finder.js` paints synchronously before any request, from three free sources in order: an exact cached `/search` answer, the device pool of the viewer's own games, and `Game.cachedSearchPrefix()` (the cached answer to a shorter query this one extends — typing "catan" asks five questions whose answers all live inside the first) filling whatever is left of the 20-row cap. **The pool is the viewer's own shelves**: `Collection.cachedShelf(me, {owned, played, wishlist})`, three map lookups into `collection.shelf`, each entry a whole shelf of up to 1000 rows with expansions already excluded server-side and a full `GameSummary` per row — precisely "games related to this user", and crucially including the ones they've *played but don't own*, which a Gather host reaches for constantly. `game.recent` and the `game.bundle` entries follow as a long-lived floor (a bundle stays peekable for 60 minutes against a shelf's 5, so a host who has had the app open a while still gets their whole library). The pool used to be bundle-only and reachable only when `BgbNet` said offline — backwards on both counts, because a host is reaching for one of their own games whether or not there's signal. The finder warms any missing shelf from an idle callback on mount (`_warmShelves`; `owned` is usually already warm from `init.js`, leaving `played` and `wishlist` as one background fetch each, once), and `Collection.invalidateShelves()` makes a mutated shelf read as missing so it refetches. Device matches are ordered prefix-before-substring, then by name; `/search` then replaces the provisional list in place. Three consequences worth knowing: **queries under 3 characters never reach the server** (pg_trgm has no trigram to match under three, so those were the two most expensive and least useful queries of every search — they're device-only now, and the empty state says "keep typing" rather than "no matches"); **a superseded keystroke aborts its request** via a caller `signal` threaded through `api.get`, and a caller abort is deliberately *not* counted as a network failure by `BgbNet` (`err.aborted`, no `noteFailure()`) or typing would walk the app into offline mode; and **provisional rows are tappable** — `_gameById` is populated by the provisional paint, so a host can pick their game before the response lands. The dimming under `.game-finder-dropdown--loading` was softened to 0.75 accordingly: those rows are real matches now, not the previous query's leftovers. `index.html` also preconnects to `APP_CONFIG.apiBase` right after `config.js`, so the first search of a session doesn't pay DNS + TLS on top of its round trip.
