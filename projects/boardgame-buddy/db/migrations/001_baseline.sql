-- ─────────────────────────────────────────────────────────────────────────────
-- boardgamebuddy 001 — baseline schema
--
-- Structure only: 23 tables with their columns, constraints, indexes, RLS and
-- grants. Collapsed from the 73-file history now in archive/, which this
-- reproduces exactly — it was generated by replaying all 73 archived
-- migrations into an empty database and reading the result back, so columns
-- added and later dropped simply are not here, and neither are the tables that
-- 018, 044 and 064 removed.
--
-- FRESH-DB ONLY. Production is already at this state. Do not run on existing DB.
-- Most statements are guarded (IF NOT EXISTS / ON CONFLICT), but the three
-- CREATE POLICY statements at the end are not, because Postgres has no
-- IF NOT EXISTS for them — so a second run of this file stops there. That is
-- the intended posture, not an oversight: this file describes a starting
-- state, it is not a repair tool for a database that has drifted.
--
-- Run first, then 002_seed.sql (reference data), then 003_rpcs.sql (functions).
-- db/migrations/_shared/003_project_roles.sql runs after every app's baseline.
--
-- Tables are ordered so foreign keys always point at something already
-- created. Where a column's history explains its shape, the note above the
-- table says so and names the archive file; the archive is where the full
-- reasoning lives.
--
-- Two conventions from .claude/rules/database-supabase.md apply throughout:
-- RLS is enabled on every table (the backend uses the service role key and
-- bypasses it — this is what blocks direct anon-key access), and every table
-- grants SELECT to boardgamebuddy_role, the per-project read-only login role.
-- Only boardgamebuddy_play_session_scores and boardgamebuddy_play_sessions go
-- further, with real policies and Data API grants, because the web client
-- reaches those two directly through supabase-js.
-- ─────────────────────────────────────────────────────────────────────────────

-- ── Project role ─────────────────────────────────────────────────────────────
-- Created with a placeholder password; _shared/003_project_roles.sql resets it
-- to a generated one and prints it once.
DO $$
BEGIN
...
-- ── Extensions ───────────────────────────────────────────────────────────────
-- pg_trgm backs the fuzzy game search and the ghost-name similarity matching.
-- It lives in the `extensions` schema on Supabase, so call sites qualify it
-- (extensions.similarity, extensions.gin_trgm_ops).
