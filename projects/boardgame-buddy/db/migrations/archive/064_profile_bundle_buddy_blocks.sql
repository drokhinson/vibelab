-- 064_profile_bundle_buddy_blocks.sql — Profile Other becomes buddy-aware.
--
-- Until now /profile/bundle handed every signed-in caller the same payload for
-- anybody's profile: stats, shelves, and the target's last N plays. Two changes
-- here, both keyed off whether viewer and target are actually buddies:
--
--   1. RECENT PLAYS ARE FOR BUDDIES. `recent_plays` is now NULL for a viewer
--      who is neither the target nor an accepted buddy of theirs. The COUNT
--      (`recent_plays_total`) stays for everyone — it is one of the four
--      general stats the profile header shows, and a total is not a log.
--   2. TWO NEW BUDDY-ONLY BLOCKS. `together` (the pair's shared record, shaped
--      like bgb_user_stats_detail's `nemesis` so the screen can draw the same
--      split bar) and `top_games` (the target's three most-played games).
--
-- Both new blocks and the is_buddy flag are NULL/false for self — Profile Self
-- has its own Stats spoke and does not read them. Because every added key is
-- additive and self-null, a cached pre-064 bundle behaves exactly as before on
-- the self hub, so bootstrap_version is deliberately NOT bumped: the wipe would
-- cost every user a cold boot to publish keys their own hub ignores. Profile
-- Other gates its render on the relation from /users/{id}/profile (uncached),
-- not on the bundle, so a stale stranger bundle in the 5-minute SWR window
-- cannot paint plays either.
--
-- `together` follows the nemesis rules for the same reasons (058): competitive
-- plays only — in co-op every seat wins or loses together, so both sides of the
-- split bar would fire on one play — and both people must be actual
-- PARTICIPANTS, not merely the logger, since a play you logged but sat out has
-- no result to attribute. There is no 3-play floor: nemesis needs one because
-- it is picking a winner out of everyone you have ever played, and this pair is
-- already chosen. `together` is NULL only when the two have never sat down
-- together competitively.
--
-- No schema change; only bgb_profile_bundle is replaced.
