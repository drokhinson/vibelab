...
- Mobile-first responsive. Max width 480px for single-column apps, 900px for dashboards. An app that grows a tablet or desktop layout keeps the phone layout untouched and adds **tiers behind a project-owned lever** — `<html data-<prefix>-layout="phone|tablet|wide|land">`, set pre-paint and owned by a `domain/layout.js` shaped like the theme controller — with the column width as a token (`--<prefix>-col-max`) that every docked bar pins to. Per-tier rules select the attribute, never a bare `min-width` media query, so a Settings pin and the viewport go through one lever. A landscape-phone tier is keyed on **height**, not width — a phone on its side passes every tablet width test — and is asked before the width ladder. Reference: boardgame-buddy (`Docs/ARCHITECTURE.md` §4.6).
...
- **An overlay has four exits and they mean the same thing.** A sheet, modal or popup closes on its ×, on a tap **outside the card** (not merely on the backdrop element — anything the overlay parks beside its card is outside it too), on Escape, and on the phone's back button / edge swipe. Back is the one that gets forgotten and the one that hurts most: an overlay is painted over a screen that never navigated, so with nothing in place back walks the page *behind* it and leaves the overlay on top of the wrong screen. Wire all four to one close path — see `.claude/rules/overlays.md` §8 for the mechanism (one history entry per overlay) and the keyboard-first ordering.
- **An overlay never focuses a text input on open.** Not the search box, not a name field, not "just this one, the list is short". It raises the software keyboard over the very list or card the user opened, and most opens end in a tap rather than typing. Focus something inside the overlay so a screen reader reads its label — the current row, the checked option, or the panel itself with `tabindex="-1"` — and let tapping the field be the opt-in. A search-backed overlay that wants to land showing results seeds the *list*, not the field. `.claude/rules/overlays.md` §5.
...
- **Heights are tokens; derived offsets are `calc()` off those tokens.** Declare `--nav-height` and `--header-height`, then derive `scroll-padding-top: calc(var(--header-height) + 8px)` so `scrollIntoView({block:"start"})` targets clear the pinned header, and derive every docked bar's offset the same way. No literal ever duplicates a measurement that lives in a token. **Widths too:** the content column is `--col-max`, and a docked bar's `max-width` reads it rather than repeating the number — boardgame-buddy carried a literal 480px at seven sites, and each one had to be found by hand the day the column stopped being 480.
...
- **Docked chrome is `position: fixed`, not `sticky`.** A sticky bottom bar is released by a padding calibration whose release point has to stay in sync with every other bottom offset in the app — and it drifts: boardgame-buddy's missed `#app`'s own `padding-bottom`, so the button climbed 64px at the end of every screen. Fixed has no release point. The recipe: `bottom: calc(var(--nav-height) + env(safe-area-inset-bottom))`, `left: var(--col-center, 50%)` + `translateX(-50%)` + `max-width: var(--col-max)` to pin to the content column rather than the viewport (the centre is a token so a side rail can shift the column), explicit `box-sizing: border-box`, a background gradient so content passes *under* the bar, and `pointer-events: none` on the wrapper with `auto` on the control.
