...
- **Destructive actions are confirmed.** Anything that loses user data — discarding a draft, deleting a play, abandoning a session, removing a friend, clearing a list — must present a confirm dialog before firing. The dialog states (a) what will be lost, (b) whether the action is reversible, and (c) offers a Cancel that is the default focus / first read order. Reuse the project's existing modal pattern (e.g. boardgame-buddy's `PolaroidPopup.confirm()`) — don't introduce per-screen ad-hoc dialogs. The destructive button uses the project's destructive accent (e.g. rust / red) so the affordance reads as dangerous at a glance. Non-destructive irreversible actions (publishing, sharing) get a confirm too unless the project decides the cost of the confirm outweighs the cost of an accidental tap.
