...
| `GEMINI_API_KEY` | Google AI Studio (aistudio.google.com → API Keys) | Travel Scrapbook place extraction — Gemini (free tier) identifies the place a saved URL is about (~500 in / ~150 out tokens per scrap, well inside the free quota). Missing key → scraps fail with `error_kind='llm'` and a retry affordance; nothing else breaks. | `shared-backend/routes/travel_scrapbook/services/llm.py` |
