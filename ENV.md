...
| `ANTHROPIC_API_KEY` | Anthropic Console (console.anthropic.com → API Keys) | Travel Scrapbook place extraction — Claude Haiku identifies the place a saved URL is about (~$0.001/scrap, pay-as-you-go). Missing key → scraps fail with `error_kind='llm'` and a retry affordance; nothing else breaks. | `shared-backend/routes/travel_scrapbook/services/llm.py` |
