...
| `ADMIN_API_KEY` | Hand-generated (operator) | Bearer token for admin endpoints. Missing key → every admin call is rejected (there is no fallback key) | `shared-backend/auth.py` |
...
| `BGB_QR_SECRET` | `python -c "import secrets; print(secrets.token_urlsafe(32))"` — must be ≥32 bytes, or PyJWT warns it is under the HMAC-SHA256 minimum | Signs BoardgameBuddy's short-lived add-a-buddy QR tokens. **Set this in Railway before deploying.** Rotating it is also the revocation lever: it invalidates every outstanding code at once, which is why the tokens carry no server-side state. Missing key → the QR endpoints answer 503 (there is no fallback secret). | `shared-backend/routes/boardgame_buddy/constants.py`, read by `services/buddy_qr_service.py` |
| `BGB_VAPID_PUBLIC_KEY` | `python -c "from py_vapid import Vapid; from cryptography.hazmat.primitives import serialization as s; import base64; v=Vapid(); v.generate_keys(); print(base64.urlsafe_b64encode(v.public_key.public_bytes(s.Encoding.X962, s.PublicFormat.UncompressedPoint)).decode().rstrip('='))"` (65 bytes, base64url) | The VAPID public key BoardgameBuddy's web push identifies itself with. Served to clients by `GET /push/config`; the browser passes it to `pushManager.subscribe` as `applicationServerKey`, which is what binds a subscription to this server. Safe to publish — it is the half clients hold. **Unset = the whole push feature is off**: `push_service.enabled()` is false, every send returns immediately, and the Settings card renders "not switched on for this server yet". That is the intended state for local dev. **Shape: exactly 87 base64url characters, and it starts with `B`** — that is a raw uncompressed P-256 point (65 bytes, leading `0x04`), which is the only thing `pushManager.subscribe` accepts. Anything else fails in the browser as `The provided applicationServerKey is not valid`: SPKI/DER (122 chars, starts `MFkwEwYHKoZI`, and by far the commonest mistake because it is what most "export the public key" snippets give you), PEM, a compressed point (44 chars), or the private key pasted into this slot (43 chars). `push_service._valid_public_key()` rejects all of those at boot — the server logs which half is wrong and reports `enabled: false` rather than letting a bad key reach a user's phone. | `shared-backend/routes/boardgame_buddy/constants.py`, read by `services/push_service.py` and `push_routes.py` |
| `BGB_VAPID_PRIVATE_KEY` | Same command, printing `base64.urlsafe_b64encode(v.private_key.private_numbers().private_value.to_bytes(32,'big'))` (32 bytes, base64url) — **generate it in the SAME run as the public key above; the two are one keypair and a mismatched pair fails every send.** | Signs the JWT in each push request's `Authorization: vapid` header. Never leaves the backend. **Rotating the pair invalidates every existing subscription** — a browser binds its subscription to the public key it subscribed with, so old rows keep looking valid and simply stop being delivered. Rotation therefore means: set both values, then `TRUNCATE boardgamebuddy_push_subscriptions`, and every client re-subscribes on its next launch (`domain/push.js#syncOnBoot`). Never rotate one of the pair alone. **Shape: exactly 43 base64url characters** — the raw 32-byte scalar, not a PEM or a DER blob. An unparseable value switches push off the same way an absent one does (and says so in the log), rather than being discovered one silent non-delivery at a time. | `shared-backend/routes/boardgame_buddy/constants.py`, read by `services/push_service.py` |
| `BGB_VAPID_SUBJECT` | Set by hand (default `mailto:dev@example.com`) | `mailto:` or `https://` contact the Web Push spec requires in the VAPID claims, so a push service can reach the operator if this server starts misbehaving. Some push services reject a token without a usable one, so set a real address before relying on push in production. | `shared-backend/routes/boardgame_buddy/constants.py`, read by `services/push_service.py` |
...
| `BGG_WEB_USER_AGENT` | Set by hand (defaults to a current Chrome UA string) | User-Agent for requests to boardgamegeek.com's own web endpoints (the collection write form) — NOT for xmlapi2, which keeps `vibelab-boardgame-buddy/1.0`. Those `.php` endpoints sit behind Cloudflare, which screens POSTs on how browser-shaped they look and answers a failure with a 403 the BGG app never sees. Env-tunable because the UA that gets through is a moving target. | `shared-backend/routes/boardgame_buddy/bgg_client.py` |
| `BGG_THROTTLE_SECONDS` | Set by hand (default `1.5`) | Seconds between BGG collection *reads* inside a sweep — the eight subtype x status-flag requests a comparison or an import makes. Also paces the comparison checklist, so raising it locally is how you watch the sweep advance one batch at a time without a huge BGG account. | `shared-backend/routes/boardgame_buddy/bgg_collection_read.py` |
...
| `GEMINI_API_KEY` | Google AI Studio (aistudio.google.com → API Keys) | Shared Gemini key (free tier). **Travel Trove** place extraction — identifies the place a saved URL is about (~500 in / ~150 out tokens per scrap); missing key → scraps fail with `error_kind='llm'` and a retry affordance. **BoardgameBuddy** chapter drafting — the create wizard's "Generate" button drafts a reference-guide chapter (~1k in / ~600 out tokens per press), or, for a scoring-grid chapter, the rows of its score sheet (~700 in / ~500 out); missing key → the button 502s with a toast and the step's Skip still works. **BoardgameBuddy** play import — Settings → Import plays reads a pasted note (a tally list, a Notes table) into draft plays for the wizard to review (~5k in / up to 8k out tokens per parse, once per import); missing key → the parse step 502s and the wizard cannot advance. Nothing else breaks in either app. | `shared-backend/gemini.py` (shared caller), `shared-backend/routes/travel_scrapbook/services/llm.py`, `shared-backend/routes/boardgame_buddy/services/chapter_ai.py`, `shared-backend/routes/boardgame_buddy/services/chapter_grid_ai.py`, `shared-backend/routes/boardgame_buddy/services/play_import_ai.py` |
