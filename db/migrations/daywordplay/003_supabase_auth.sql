-- ─────────────────────────────────────────────────────────────────────────────
-- daywordplay — switch user identity to Supabase Auth.
--
-- Drops the legacy daywordplay_users table (custom JWT + bcrypt) and every
-- user-FK'd table CASCADE, then recreates them pointing at
-- public.daywordplay_profiles (PK = auth.users.id).
--
-- Vocabulary seed (daywordplay_words / proposed_words.word data) is preserved
-- by *not* dropping daywordplay_words. proposed_words is dropped because its
-- proposed_by FK targets daywordplay_users — pending proposals are wiped.
--
-- WIPES all user-owned data. Production has not deployed users yet.
...
GRANT SELECT ON public.daywordplay_profiles TO daywordplay_role;
...
CREATE INDEX idx_daywordplay_groups_code ON public.daywordplay_groups(code);
...
CREATE INDEX idx_daywordplay_group_members_user  ON public.daywordplay_group_members(user_id);
CREATE INDEX idx_daywordplay_group_members_group ON public.daywordplay_group_members(group_id);
...
CREATE INDEX idx_daywordplay_daily_words_lookup
...
CREATE INDEX idx_daywordplay_sentences_date
...
CREATE INDEX idx_daywordplay_votes_sentence ON public.daywordplay_votes(sentence_id);
...
CREATE INDEX idx_daywordplay_bookmarks_user ON public.daywordplay_bookmarks(user_id);
...
CREATE INDEX daywordplay_proposed_words_status_idx
...
CREATE INDEX idx_daywordplay_join_requests_group ON public.daywordplay_join_requests(group_id);
CREATE INDEX idx_daywordplay_join_requests_user  ON public.daywordplay_join_requests(user_id);
