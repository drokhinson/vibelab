-- ─────────────────────────────────────────────────────────────────────────────
-- BoardgameBuddy — perf pass: one RPC for the Gather player picker
--
-- The picker's three lists were three endpoints, twelve round trips, and one
-- of the last places the app still counted rows in Python:
--
--   GET /played-with    (played_with_service.fetch_played_with)      5 trips
--       SELECT every play id the viewer logged
--       SELECT every play id the viewer appears in
--       SELECT all play_players for that (unbounded!) id list
--       count them in a Python dict, then SELECT the profiles
--       SELECT the buddy edges for the relation flags
--   GET /ghost-players  (fetch_ghost_players)                        2 trips
--   GET /buddies        (buddy_service.list_accepted_buddies)        2 trips
--
-- The id list is unbounded — a BGG-synced account ships thousands of UUIDs into
-- Python and back out in an `in_()` filter, per call. This is also the slowest
-- of the six blocks GET /bootstrap gathers, and a gather's wall time is its
-- slowest block, so this one chain set the floor on first load.
--
-- bgb_play_partners does all three as SQL aggregates in one call and returns
-- {accounts, ghosts, recent} — the exact shape Buddy.allBuddies() caches and
-- bootstrap seeds, with keys matching BuddyEdgeResponse / GhostPlayer /
-- PlayedWithUser so they validate straight through.
--
-- Semantics are a deliberate port of the Python, including two asymmetries
-- that are easy to "tidy" into bugs:
--   • `recent` spans plays the viewer LOGGED **or** APPEARS IN; `ghosts` covers
--     only plays the viewer LOGGED. (Someone else's ghost names are not the
--     viewer's to reuse.)
--   • ghost grouping is CASE-SENSITIVE on the trimmed name, matching the Python
--     dict key — "Dave" and "dave" stay two entries.
-- ─────────────────────────────────────────────────────────────────────────────
