-- sauceboss — support multiple input steps per step
--
-- A step can now combine output from multiple prior steps.
-- `input_from_steps INT[]` holds the full list; `input_from_step` is kept for
-- backward compatibility with the native app (always reflects the first element).

-- ── Add column ──────────────────────────────────────────────────────────────
ALTER TABLE public.sauceboss_sauce_step
  ADD COLUMN IF NOT EXISTS input_from_steps INT[] NOT NULL DEFAULT '{}';

-- ── Backfill from existing single-value column ──────────────────────────────
UPDATE public.sauceboss_sauce_step
   SET input_from_steps = ARRAY[input_from_step]
 WHERE input_from_step IS NOT NULL
   AND input_from_steps = '{}';


-- ── Replace create_sauceboss_sauce ──────────────────────────────────────────
...
BEGIN
  ...
  LOOP
    -- Build input_from_steps array: prefer inputFromSteps, fall back to wrapping inputFromStep
    v_ifs_arr := COALESCE(
...
-- ── Replace update_sauceboss_sauce ──────────────────────────────────────────
...
-- ── Replace get_sauceboss_all_sauces_full to include inputFromSteps ─────────
...
-- ── Update get_sauceboss_sauces_for_target to include inputFromSteps ────────
-- (Replaces version from 014_release_compat.sql)
...
-- ── Update fork_sauceboss_sauce to copy input_from_steps ────────────────────
