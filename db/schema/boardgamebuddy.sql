...
CREATE TABLE IF NOT EXISTS public.boardgamebuddy_profiles (
  ...
  -- follows the person; the subscriptions it gates are per device. On by
  -- default since migration 018 — the column is intent, and nothing is
  -- delivered until a device subscription exists, which needs a browser
  -- permission grant.
  push_tier TEXT DEFAULT 'all'::text NOT NULL,
