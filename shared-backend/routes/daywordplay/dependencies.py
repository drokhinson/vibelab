"""FastAPI dependencies for Day Word Play — Supabase Auth-backed user resolution."""
...
class CurrentUser(BaseModel):
    """App-level user context for Day Word Play."""
    user_id: str
    display_name: str
    is_admin: bool = False
...
async def get_current_user(
    ...
    """Resolve a Supabase Auth user to a DayWordPlay profile (auto-creates on first login)."""
    sb = get_supabase()
    result = (
    ...
    display_name = (su_user.email or "").split("@")[0] or "Player"
    sb.table("daywordplay_profiles").insert({
        "id": su_user.sub,
